...
public class ProfileRESTInterface extends SuperRestInterface {
	...
	static void setupProfileRESTInterface() {
		...
		Spark.put(OpenAPEEndPoints.USER_PASSWORD, (req, res) -> {
			...
			try {
				pwChangeReq = (PasswordChangeRequest) SuperRestInterface.extractObjectFromRequest(req,
			...
			final DatabaseObject result = databaseConnection.getByUniqueAttribute(MongoCollectionTypes.USERS,
