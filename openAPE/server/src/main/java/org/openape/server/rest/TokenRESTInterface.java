...
class TokenRESTInterface extends SuperRestInterface {
	...
	/**
	 * Register a new route POST /token to get a token in exchange for a
	 * username and password.
	 */

	static void setupTokenRESTInterface(final AuthService authService) {
		Spark.post("/token", (req, res) -> {
			...
			// Return 400 Bad Request if the grant_type parameter is not set to
			// password
