...
public class ResourceList {
	...
	public String addResource(final FileItem resource, final String mimeType)
		...
		if (!databaseConnection.storeMimeType(fileName, mimeType)) {
			throw new IOException(Messages.getString("ResourceList.MimeTypeCouldNotBeStoredErrorMsg"));//$NON-NLS-1$
		}
		...
		try {
			...
			final File fileToWrite = new File(ResourceList.RESOURCEFOLDERPATH + File.separator + fileName);
	 ...
	 * @throws IOException
	 */
	public boolean deleteResource(final String fileName) throws IllegalArgumentException, IOException {
		if (this.resourceExists(fileName)) {
	 ...
	 */
	public GetResourceReturnType getResoureFile(final String fileName) throws IllegalArgumentException, IOException {
		if (this.resourceExists(fileName)) {
