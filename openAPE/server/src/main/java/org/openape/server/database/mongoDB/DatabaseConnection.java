...
public class DatabaseConnection implements ServerMonitorListener {
	...
	public UpdateResult updateDocument(final MongoCollectionTypes type, final String id, final String indexName,
			final String indexValue) throws Exception {
	...
	public String getMimeType(final String fileName) throws IOException {
		...
		if (!resultInterator.hasNext()) {
			...
			try {
				...
				final DatabaseObject mimeTypeObject = mapper.readValue(jsonResult, MimeTypeDatabaseObject.class);
	 ...
	 * @throws IOException
	 */
	private DatabaseObject executeQuery(final MongoCollectionTypes type, final MongoCollection collection,
	 ...
	 */
	public boolean storeMimeType(final String fileName, final String mimeType)
			throws IOException, IllegalArgumentException {
		// check if key is in use.
		if (this.getMimeType(fileName) != null) {
			throw new IllegalArgumentException(Messages.getString("ResourceList.FilenameInUseErrorMassage"));
		}
		final MongoCollection<Document> collectionToWorkOn = this
				.getCollectionByType(MongoCollectionTypes.RESOURCEMIMETYPES);
