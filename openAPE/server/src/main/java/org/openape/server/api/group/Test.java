...
public class Test {
	...
	public static void main(final String[] args) throws IOException {
		...
		// MongoClientURI uri = new MongoClientURI("mongodb://openAPE:1234@localhost/?authSource=openAPE");
		...
		// query.put("members.state", new Document("$in", Arrays.asList("MEMBER", "ADMIN")));
		 ...
		 * BasicDBList elemMatch = new BasicDBList();
		 * BasicDBObject memberCondition1 = new BasicDBObject("userId", "12");
		 * BasicDBObject memberCondition2 = new BasicDBObject("state", new Document("$in", Arrays.asList("MEMBER",
		 * "ADMIN")));
		 * elemMatch.add(memberCondition1);
		 * elemMatch.add(memberCondition2);
		...
		final List groups = databaseConnection.getDocumentsByQuery(MongoCollectionTypes.GROUPS, query4, true);
		...
		// List<Document> documents = databaseConnection.getAllDocuments(MongoCollectionTypes.USERS);
		// System.out.println(documents.get(0).getObjectId("_id"));
		// User user = (User)databaseConnection.getByUniqueAttribute(MongoCollectionTypes.USERS, "username", "user1");
