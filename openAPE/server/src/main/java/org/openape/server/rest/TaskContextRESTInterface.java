...
public class TaskContextRESTInterface extends SuperRestInterface {
	...
	public static void setupTaskContextRESTInterface(final TaskContextRequestHandler requestHandler,
		 ...
		 */
		Spark.post(Messages.getString("TaskContextRESTInterface.TastContextURLWithoutID"), (req, res) -> { //$NON-NLS-1$
			...
			try {
				...
				// object.
				final TaskContext receivedTaskContext = (TaskContext) SuperRestInterface.extractObjectFromRequest(req,
						TaskContext.class);
				...
				final String taskContextId = requestHandler.createTaskContext(receivedTaskContext);
		 ...
		 * identified by ID.
		 */
		Spark.get(Messages.getString("TaskContextRESTInterface.TastContextURLWithID"), (req, res) -> { //$NON-NLS-1$
			final String taskContextId = req.params(Messages.getString("TaskContextRESTInterface.IDParam")); //$NON-NLS-1$
			try {
				// if it is successful return task context.
				final TaskContext taskContext = requestHandler.getTaskContextById(taskContextId);
				// Make sure only admins or the owner can view the context,
		 ...
		 */
		Spark.delete(Messages.getString("TaskContextRESTInterface.TastContextURLWithID"), (req, res) -> { //$NON-NLS-1$
			final String taskContextId = req.params(Messages.getString("TaskContextRESTInterface.IDParam")); //$NON-NLS-1$
			try {
