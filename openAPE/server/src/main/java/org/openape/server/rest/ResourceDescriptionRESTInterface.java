...
public class ResourceDescriptionRESTInterface extends SuperRestInterface {
	...
	public static void setupResourceDescriptionRESTInterface(final ResourceDescriptionRequestHandler requestHandler,
			final AuthService auth) {

		// Authentication: Make sure only registered principals (users and
		// admins) can create a new resource description
		Spark.before(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithoutID"),
				auth.authorize("user"));
		// Authentication: Everyone can access the route for a specific resource
		// description
		Spark.before(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithID"),
				auth.authorize("anonymous"));

		/**
		 * Request 7.7.2 create resource description.
		 */
		Spark.post(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithoutID"), //$NON-NLS-1$
				(req, res) -> {
					try {
		...
		Spark.get(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithID"), //$NON-NLS-1$
				(req, res) -> {
					...
					try {
						...
						// resource description, except if it is public
		 ...
		 */
		Spark.get(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionFromListingURL"), //$NON-NLS-1$
				(req, res) -> {

					// Currently not used parameter.
					// final String index = req.params(":index");
					final String listingId = req
							.params(Messages.getString("ResourceDescriptionRESTInterface.listingIDParam")); //$NON-NLS-1$
		 ...
		 */
		Spark.put(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithID"), //$NON-NLS-1$
				(req, res) -> {
					if (!req.contentType().equals(Messages.getString("MimeTypeJson"))) {//$NON-NLS-1$
						res.status(SuperRestInterface.HTTP_STATUS_BAD_REQUEST);
						return Messages.getString("Contexts.WrongMimeTypeErrorMsg");//$NON-NLS-1$
					}
					final String resourceDescriptionId = req
							.params(Messages.getString("ResourceDescriptionRESTInterface.IDParam")); //$NON-NLS-1$
					try {
						final ResourceDescription receivedResourceDescription = (ResourceDescription) SuperRestInterface
								.extractObjectFromRequest(req, ResourceDescription.class);
						// Test the object for validity.
		 ...
		 */
		Spark.delete(Messages.getString("ResourceDescriptionRESTInterface.ResourceDescriptionURLWithID"), //$NON-NLS-1$
				(req, res) -> {
					final String resourceDescriptionId = req
							.params(Messages.getString("ResourceDescriptionRESTInterface.IDParam")); //$NON-NLS-1$
					try {
