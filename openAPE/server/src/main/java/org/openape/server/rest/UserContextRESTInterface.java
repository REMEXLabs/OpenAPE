...
public class UserContextRESTInterface extends ContextRestInterface {
	...
	private static UserContext createRequestObejct(final Request req) throws IllegalArgumentException {
	...
	public static void setupUserContextRESTInterface(final UserContextRequestHandler requestHandler,
		 ...
		 * identified by ID.
		 */
		Spark.get(Messages.getString("UserContextRESTInterface.UserContextURLWithID"), (req, res) -> //$NON-NLS-1$

		{
			...
			try { // if it is successful return user context.
				...
				final String jsonData = UserContextRESTInterface.createReturnString(req, userContext);
				return jsonData; // if not return corresponding error status.
		...
		Spark.put(Messages.getString("UserContextRESTInterface.UserContextURLWithID"), (req, res) -> {
			final String userContextId = req.params(Messages.getString("UserContextRESTInterface.IDParam")); //$NON-NLS-1$
			...
			try {
				...
				receivedUserContext.getImplementationParameters()
						.setOwner(userContext.getImplementationParameters().getOwner()); // Make // sure // the // owner
																							// //
																							// can't // be // changed //
																							// Perform the update
			...
			} catch (JsonParseException | JsonMappingException | IllegalArgumentException e) { // If the parse or update
																								// is
		...
		Spark.delete(Messages.getString("UserContextRESTInterface.UserContextURLWithID"), (req, res) -> //$NON-NLS-1$
		{ // $NON-NLS-1$

			String userContextId = req.params(Messages.getString("UserContextRESTInterface.IDParam")); //$NON-NLS-1$
			try { // Check if the user context does exist

				UserContext userContext = requestHandler.getUserContextById(userContextId); // Make sure only admins and
																							// the owner can delete a //
																							// context
				auth.allowAdminAndOwner(req, res, userContext.getImplementationParameters().getOwner()); // Perform
																											// delete
																											// and
																											// return
																											// empty
		...
		createContextListRestEndpoint(Messages.getString("UserContextRESTInterface.UserContextURLWithoutID"),
