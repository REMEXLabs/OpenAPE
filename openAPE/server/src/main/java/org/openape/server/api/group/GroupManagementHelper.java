...
public class GroupManagementHelper {
	 ...
	 *            map which contains all user names as key and all user IDs as corresponding values. It must not be
	 *            null!
		...
		if ((groupMembersAsString.isEmpty() == false)
			...
			for (final String line : lines) {
				...
				if (line.matches(regexLine) == false) {
					...
					// String userMessage = TemplateFiller.fillTemplate(LanguageHandler.getWord(locale,
					// "WEB_GROUPS_NEW_GROUP_PAGE_ERROR_MESSAGE_INVALID_LINE"), line);
				...
				if (userNameToUserIdMap.containsKey(userName)) {
					...
					// String userMessage = TemplateFiller.fillTemplate(LanguageHandler.getWord(locale,
					// "WEB_GROUPS_NEW_GROUP_PAGE_ERROR_MESSAGE_UNKNOWN_USER_NAME"), userName);
					// throw new BadRequestException(message, null, userMessage);
				}
	 ...
	 *            map which contains all user IDs as key and all user names as corresponding values. It must not be
	 *            null!
