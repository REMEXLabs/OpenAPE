...
public class EquipmentContextRESTInterface extends SuperRestInterface {
	...
	public static void setupEquipmentContextRESTInterface(final EquipmentContextRequestHandler requestHandler,
			final AuthService auth) {

		// Authentication: Make sure only registered principals (users and
		// admins) can create a new context
		Spark.before(Messages.getString("EquipmentContextRESTInterface.EquipmentContextURLWithoutID"),
				auth.authorize("user"));
		 ...
		 */
		Spark.post(Messages.getString("EquipmentContextRESTInterface.EquipmentContextURLWithoutID"), (req, res) -> { //$NON-NLS-1$
			...
			try {
				// Try to map the received json object to an EquipmentContext
				// object.
				...
				// Make sure to set the id of the authenticated user as the
				// ownerId
				...
				final String equipmentContextId = requestHandler.createEquipmentContext(receivedEquipmentContext);
		 ...
		 */
		Spark.get(Messages.getString("EquipmentContextRESTInterface.EquipmentContextURLWithID"), //$NON-NLS-1$
				(req, res) -> {
					final String equipmentContextId = req
							.params(Messages.getString("EquipmentContextRESTInterface.IDParam")); //$NON-NLS-1$
					try {
				...
				(req, res) -> {
					...
					try {
						...
						receivedEquipmentContext.setOwner(equipmentContext.getOwner()); // Make
																						...
																						// changed
						// Perform update
						requestHandler.updateEquipmentContextById(equipmentContextId, receivedEquipmentContext);
						res.status(SuperRestInterface.HTTP_STATUS_OK);
		 ...
		 */
		Spark.delete(Messages.getString("EquipmentContextRESTInterface.EquipmentContextURLWithID"), (req, res) -> { //$NON-NLS-1$
			final String equipmentContextId = req.params(Messages.getString("EquipmentContextRESTInterface.IDParam")); //$NON-NLS-1$
			try {
				// Check if the user context does exist
