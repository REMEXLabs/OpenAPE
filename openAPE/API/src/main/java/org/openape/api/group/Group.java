 ...
 * the resources. For each resource it is defined which group is allowed to read, update and delete it and change the
 * access rights for it. Thus which access
...
public class Group extends DatabaseObject {
	...
	/**
	 * Create a new group and set's the value of {@link Group#id} to null. This constructor should be used, to
	 * instantiate groups, which are not already stored in the database.
	 *
	 * @param name
	 ...
	 *
	 * @return the group's id, which is a string. If the group is not already stored in the database, null can be
	 *         returned.
	 */
	public String getId() {
		return this.id;
	...
	/**
	 * Setter for the group's id. It must not be empty and should not be null. Null is only allowed, if the group is not
